...
def writeToA():
    ...
    def fn(s:str):
        if s.strip() == "" or s.strip() == "SCHEDULE OF CLASSES - FALL 2023" or re.match("^John Abbott College", s) or re.match("^SECTION", s):
            return False
        return True
    ...
    global out 
    out = list(map(lambda i: {i: len(i) - len(i.strip())}, raw))
...
def writeToOut():
    ...
    def handleSectionChange(section, disc, lecture, lab, more, code, program, course, codeHeader, count):
        Sections.append({
            "count": count,
            "program": program,
            "course": course,
    ...
    for index, i in enumerate(raw):
        ...
        if space >= 30 and not any([re.match(timeFormat, j) for j in a]):
            if text != program:
                handleSectionChange(section, disc, lecture, lab, more, code, program, course, codeHeader, count)
                section = ""
                disc = ""
                lecture = {}
                lab = {}
