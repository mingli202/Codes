...
public class Grid<T>
{
    ...
    /// </summary>
    protected readonly IListOf<IListOf<T>> _grid;
    ...
    public Grid(IListOf<IListOf<T>> grid)
    {
    ...
    /// Gets the element at the specified row and column.
    /// </summary>
    /// <param name="p">The row and column of the element to get.</param>
    /// <returns>The element at the specified row and column.</returns>
    public Option<T> Get(Point p) =>
        _grid.Get(p.Row).AndThen((rowCells) => rowCells.Get(p.Col));
    ...
    /// Sets the element at the specified row and column.
    /// </summary>
    /// <param name="p">The row and column of the element to set.</param>
    /// <param name="value">The value to set.</param>
    /// <returns>A new grid with the element set.</returns>
    public Grid<T> Set(Point p, T value) =>
        _grid
            .Get(p.Row)
            .MapOr(this, (rowGrid) => new Grid<T>(_grid.Set(p.Row, rowGrid.Set(p.Col, value))));
    ...
    /// <param name="col2">The column of the second element to swap.</param>
    /// <returns>A new grid with the elements swapped.</returns>
    public Grid<T> Swap(Point p1, Point p2) =>
        Get(p1)
            .MapOr(
                this,
                (el1) =>
                    Get(p2).MapOr(this, (el2) => Set(p1, el2).Set(p2, el1))
            );
    ...
    private static bool IsValidRectangle(IListOf<IListOf<T>> grid) =>
        grid.Get(0)
    ...
    public IListOf<IListOf<T>> ToListOfList() => _grid;

    public bool Equals(Grid<T> other) => other._grid.Equals(_grid);
}
